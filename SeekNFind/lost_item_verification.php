...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $query = "SELECT name FROM lost_items WHERE item_id = ?";
    $stmt = $con->prepare($query);
    ...
    if (move_uploaded_file($_FILES["verification_file"]["tmp_name"], $targetFilePath)) {
        // Prepare and bind
        $query = "INSERT INTO lost_items_claim (item_id, user_id, submitter_name, email_address, name, description, date_found, time_found, location_found, picture_path, claim_status, claim_type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', 'lost')";
        $stmt = $con->prepare($query);
        ...
        if ($stmt) {
            ...
            if ($stmt->execute()) {
                ...
                try {
                    ...
                    // Content
                    $mail->Subject = 'Item Verification';
                    $mail->Body = "Good Day! Thank you for providing proof of ownership of the item. Please go to the Lost and Found Room to continue the verification process. Always check your Email for any updates on the status of the item. If you have any further questions or concerns, please do not hesitate to reach out to us.";
